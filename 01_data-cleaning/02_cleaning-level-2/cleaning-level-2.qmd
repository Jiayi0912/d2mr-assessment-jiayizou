...
# Compare the messy and original midwest datasets
all.equal(messy.midwest, midwest)
```
Using the `all.equal()` function, we can see a total of `r length(all.equal(messy.midwest, midwest))` differences between the two datasets. Specifically, compared to the original dataset, in the messy midwest dataset:
- Column order is different, and the messy dataset includes an extra variable.
- 21 column names are messy, including problems of capitalization, extra spaces, typos, and adding units.
- Data types are all stored as character or externalptr, while many of them are numeric in the original dataset.
- Values in the county and state column have lots of problems (extra spaces, capitalization, typos, etc.)
- The first 27 columns include missing or extra rows.
